...
const App: React.FC = () => {
  ...
  const [mainTab, setMainTab] = useState('space');
      ...
      if (configRes.ok) {
        ...
        setConfig(prev => {
          ...
          if (globalConfig && Object.keys(globalConfig).length > 0) {
            ['script', 'image', 'video', 'videoVeoFast', 'videoSeedance', 'gptImage'].forEach(key => {
  ...
  const deductPoints = async (amount: number, reason: string): Promise<{ success: boolean; error?: string }> => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (historyRes.status === 401 || pipelinesRes.status === 401) {
        handleLogout();
        return;
      }
      ...
      if (historyRes.ok) {
        ...
        if (Array.isArray(history)) {
          setSmartHistory(prev => {
            ...
            const localBaseline = prev.length > 0 ? prev : (() => {
              const saved = localStorage.getItem('smartHistory');
              return saved ? JSON.parse(saved) : [];
            })();

            // 2. Start with server data
      ...
      <div className="h-full flex overflow-hidden relative">
        ...
        <div className="h-full w-full overflow-hidden flex flex-col border-r border-gray-100">
          <div className="h-full w-full relative">
            ...
            <div className={mainTab === 'mycompany' ? 'contents' : 'hidden'}>
              <MyCompany 
                key={`mycompany-persistent-${user?.id || 'guest'}`}
                userId={user?.id}
                config={config}
                userPoints={user?.points || 0}
                deductPoints={deductPoints}
            ...
            <AnimatePresence mode="wait">
              {!['space', 'mycompany'].includes(mainTab) && (
                <motion.div
                  key={mainTab}
