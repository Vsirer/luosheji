...
## 四、 角色空间一致性与连续性规则
为了保持镜头变换时角色空间位置和状态的连贯，必须遵循以下规则：
1. **背景与物件锁定**：在描述中锁定场景的主要物件和角色的相对位置。没有明确移动动作（如起身、迈步、走过去）的描述，人物在后续镜头中不能凭空瞬间移动位置。
2. **动作连贯过渡**：如果角色需要变换姿态或位置，必须在描述中加入过渡性动作词，确保动作具有明显的物理连贯性和过渡感。
3. **语言与特征一致**：在所有镜头中，角色形象及所穿服饰、所持道具在没有剧情变动前必须保持一致。
...
## 六、角色与空间锁定规则 (Seedance 2.0 专属)
为了解决镜头变动时角色空间站位变化、穿帮或是床上/地上虚空移动位移不连贯的问题，必须严格遵循以下站位锁定规则：
1. **真实空间连续性**：在前后镜头中，角色形象及服饰、道具在没有剧本明确变动描述前必须保持完全一致。未描述起身、迈步等动作时，禁止人物凭空瞬间位移。
2. **镜头统一句式格式**：
   在分段提示词中所包含的每一个镜头描述，必须在景别/运镜标注后（即冒号后面），直接接上该镜头具体的画面动作描述：
   - 格式：\`开始秒–结束秒s（景别、拍摄距离、拍摄方位与角度、运镜）：[具体角色及画面动作描述]\`。
3. **坐标偏移防抖提醒**：当片段中累计包含大于或等于 8 个镜头时，在 \`assets.continuity\` (连续性说明) 字段中最后自动增加一句：\`[防抖建议] 累计镜头超8镜，建议复用首帧参考图或重新框选更新重置坐标系。\`
