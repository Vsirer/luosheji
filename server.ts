...
import { testOSSConnection, updateOSSConfig } from "./services/oss";
import { persistFromBase64, persistFromUrl } from "./services/storage";
import { execSync } from "child_process";
import crypto from "crypto";

const __filename_path = (typeof __filename !== 'undefined' && __filename)
  ? __filename 
  : (typeof import.meta !== 'undefined' && typeof import.meta.url === 'string') 
    ? fileURLToPath(import.meta.url) 
    : path.join(process.cwd(), 'server.ts');
const __dirname_path = path.dirname(__filename_path);
...
async function startServer() {
  ...
  const authenticateToken = async (req: any, res: any, next: any) => {
    ...
    jwt.verify(token, getJwtSecret(), async (err: any, user: any) => {
      ...
      // Fallback: If the user email matches the owner, ensure they have admin role
      // This helps if the database role was not correctly set during registration
      const lowerUsername = user.username?.toLowerCase() || '';
      const adminEmails = [
        'stxuxiaoyang@gmail.com',
        'zhurui',
        'bluedream.hebe@gmail.com'
      ].map(e => e.toLowerCase());

      console.log(`[Auth] Checking admin fallback for: "${lowerUsername}". Admin list: ${JSON.stringify(adminEmails)}`);

      if (lowerUsername && adminEmails.includes(lowerUsername)) {
  ...
  app.get("/api/user/profile", authenticateToken, async (req: any, res) => {
    ...
    let teamInfo = null;
    if (user.leader_id) {
      const [leaders]: any = await db.query("SELECT id, username, points FROM users WHERE id = ?", [user.leader_id]);
      if (leaders[0]) {
        teamInfo = {
  ...
  app.post("/api/user/deduct-points", authenticateToken, async (req: any, res) => {
    ...
    try {
      ...
      console.log(`[Points] User ${req.user.id} (${user.username}) attempting to deduct ${deductAmount}. Current points: ${user.points}, Leader ID: ${user.leader_id}, Point Limit: ${user.point_limit}`);
      ...
      if (user.leader_id) {
        ...
        console.log(`[Points] Checking leader ${user.leader_id} (${leader?.username}). Leader points: ${leader?.points}`);
        
        if (leader && leader.points >= deductAmount) {
          targetId = user.leader_id;
    ...
    try {
        ...
        for (const [key, value] of Object.entries(newObj)) {
          if (typeof value === 'string' && isNotOSS(value)) {
            ...
            try {
              ...
              console.error(`>>> [DEBUG] Failed to upload ${prefix}.${key} to OSS:`, err.message);
              throw new Error(`嵌套媒体保存到OSS失败 (${prefix}.${key}): ${err.message}`);
            }
      ...
      if (finalImageUrl && isNotOSS(finalImageUrl)) {
        ...
        try {
          ...
          console.error('Failed to persist top-level image to OSS:', err.message);
          throw new Error(`图片保存到OSS失败: ${err.message}`);
        }
      }
      ...
      if (finalVideoUrl && isNotOSS(finalVideoUrl)) {
        ...
        try {
          ...
          console.error('Failed to persist top-level video/audio to OSS:', err.message);
          throw new Error(`媒体保存到OSS失败: ${err.message}`);
        }
      }
      ...
      const [result]: any = await db.query(
        `INSERT INTO history (id, user_id, type, status, image_url, video_url, ark_original_url, revised_prompt, is_optimized, error, config, timestamp, position, hidden_from_canvas, parent_id) 
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) 
        ...
        [
          item.id, req.user.id, item.type, item.status, finalImageUrl, finalVideoUrl, item.arkOriginalUrl || null,
          item.revisedPrompt, item.isOptimized ? 1 : 0, item.error, JSON.stringify(finalConfig), 
          item.timestamp, JSON.stringify(item.position || null), item.hiddenFromCanvas ? 1 : 0, item.parentId || null
        ]
      );
  ...
  app.post("/api/user/leave-team/:teamId", authenticateToken, async (req: any, res) => {
    ...
    try {
      ...
      if (otherTeams.length === 0) {
        await db.query("UPDATE users SET leader_id = NULL WHERE id = ? AND leader_id = ?", [userId, team[0].leader_id]);
      }
  ...
  app.delete("/api/leader/teams/:teamId/remove-member/:userId", authenticateToken, async (req: any, res) => {
    ...
    try {
      ...
      if (otherTeams.length === 0) {
        await db.query("UPDATE users SET leader_id = NULL WHERE id = ? AND leader_id = ?", [userId, teams[0].leader_id]);
      }
  ...
  app.delete("/api/leader/remove-member/:id", authenticateToken, isLeader, async (req: any, res) => {
    try {
      ...
      await db.query("UPDATE users SET leader_id = NULL WHERE id = ? AND leader_id = ?", [memberId, leaderId]);
      res.json({ success: true });
    } catch (e: any) {
      res.status(500).json({ error: "移除成员失败", details: e.message });
    }
  ...
  app.post("/api/leader/transfer-role/:id", authenticateToken, isLeader, async (req: any, res) => {
    try {
      ...
      try {
        // 1. Set member as leader
        await connection.query("UPDATE users SET role = 'leader', leader_id = NULL WHERE id = ?", [memberId]);
  ...
  app.post("/api/admin/accounts/:id/leave-team", authenticateToken, isAdmin, async (req, res) => {
    ...
    try {
      ...
      await db.query("UPDATE users SET leader_id = NULL WHERE id = ?", [id]);
  ...
  app.get("/api/group-chats", authenticateToken, async (req: any, res) => {
    try {
      ...
      if (req.user.role === 'admin' || req.user.role === 'leader') {
        // Admins and leaders only see groups they created themselves
        query += ` WHERE g.leader_id = ? `;
  ...
  app.post("/api/group-messages", authenticateToken, async (req: any, res) => {
    ...
    try {
      ...
      if (!members[0]) return res.status(401).json({ error: "您不是该群聊的成员" });
      ...
      try {
        ...
        const agentMap: Record<string, string> = {
          'CEO': 'ceo', '导师': 'ceo', '大脑': 'ceo', 'AI': 'ceo', '统筹': 'ceo',
          '编剧': 'script', '创意': 'script', '大纲': 'script', '灵感': 'script',
...
直接输出回复内容，不要带任何前缀。`;
              ...
              const slot = respondingAgentId ? agentToSlotMap[respondingAgentId] || 'script' : 'script';
    ...
    try {
      ...
      const parsedResults = messages.map((m: any) => {
        const msg: any = {
          ...
          agentName: m.agent_name || m.senderName || (m.sender_id === 0 ? 'AI专家' : '系统'),
        ...
        if (m.quoted_message_id) {
          msg.quotedMessage = {
            ...
            agentName: m.quotedAgentName || m.quotedSenderName || (m.quotedSenderId === 0 ? 'AI专家' : '系统'),
            type: m.quotedType,
            url: m.quotedUrl,
  ...
  app.get("/api/admin/metrics", authenticateToken, isAdmin, async (req, res) => {
    try {
      ...
      // Total points used (last 30 days)
      const [pointsRows]: any = await db.query(
        "SELECT COALESCE(SUM(amount), 0) as total FROM usage_logs WHERE type = 'points_spent' AND created_at >= ?",
        [dateStr]
      );
      const totalPoints = Number(pointsRows[0].total);

      // Total images generated (last 30 days)
      const [imageRows]: any = await db.query(
        "SELECT COUNT(*) as total FROM usage_logs WHERE type = 'image_gen' AND created_at >= ?",
        [dateStr]
      );

      // Total videos generated (last 30 days)
      const [videoRows]: any = await db.query(
        "SELECT COUNT(*) as total FROM usage_logs WHERE type = 'video_gen' AND created_at >= ?",
        [dateStr]
      );

      // Total text AI usage (last 30 days)
      const [textRows]: any = await db.query(
        "SELECT COUNT(*) as total FROM usage_logs WHERE type = 'text_ai' AND created_at >= ?",
        [dateStr]
      );

      // Total script generations (last 30 days)
      const [scriptRows]: any = await db.query(
        "SELECT COUNT(*) as total FROM usage_logs WHERE type = 'script_gen' AND created_at >= ?",
        [dateStr]
      );
      ...
      const [userStats]: any = await db.query(`
        SELECT 
          u.id, 
          u.username, 
          SUM(CASE WHEN l.type = 'points_spent' THEN l.amount ELSE 0 END) as points_spent,
          SUM(CASE WHEN l.type = 'image_gen' THEN 1 ELSE 0 END) as images_count,
          SUM(CASE WHEN l.type = 'gpt_image_gen' THEN 1 ELSE 0 END) as gpt_images_count,
          SUM(CASE WHEN l.type = 'video_gen' THEN 1 ELSE 0 END) as videos_count,
          SUM(CASE WHEN l.type = 'text_ai' THEN 1 ELSE 0 END) as text_ai_count,
          SUM(CASE WHEN l.type = 'script_gen' THEN 1 ELSE 0 END) as script_gen_count
        FROM users u
        LEFT JOIN usage_logs l ON u.id = l.user_id AND l.created_at >= ?
      ...
      res.json({
        summary: {
          ...
          totalImages: imageRows[0].total || 0,
          totalVideos: videoRows[0].total || 0,
          totalTextAI: textRows[0].total || 0,
          totalScriptGen: scriptRows[0].total || 0
        },
        dailyTrend,
        userStats
      });
...
async function trimVideo(src: string, startTime: number, duration: number): Promise<string> {
  ...
  app.post('/api/generate', authenticateToken, async (req, res) => {
    const { prompt, ratio, duration, resolution, referenceAssets, videoMode, image, lastFrame } = req.body;
    ...
    const globalConfig = await getGlobalApiConfig();
    const seedanceConfig = globalConfig?.videoSeedance;
    const baseUrl = seedanceConfig?.endpoint?.replace(/\/$/, '') || 'https://www.runninghub.cn/openapi/v2/rhart-video/sparkvideo-2.0/multimodal-video';
    ...
    if (!apiKey) {
      return res.status(500).json({ error: "未配置 SEEDANCE API KEY" });
    }

    // Helper to format as asset if applicable
    const formatUrl = (u: any) => {
      const urlStr = typeof u === 'string' ? u : (u?.url || u?.uri || '');
    ...
    if (referenceAssets && referenceAssets.length > 0) {
      for (const asset of referenceAssets) {
        ...
        if (asset.type === 'video') {
          if (!videoUrlList.includes(u)) videoUrlList.push(u);
        } else if (asset.type === 'audio') {
          if (!audioUrlList.includes(u)) audioUrlList.push(u);
        } else {
          if (!imageUrlList.includes(u)) imageUrlList.push(u);
        }
      }
    }
    ...
    if (image) {
      const u = formatUrl(typeof image === 'string' ? image : (image.data || image.url));
      if (u) {
        const existingIdx = imageUrlList.indexOf(u);
        if (existingIdx !== -1) {
          firstFrameIdx = existingIdx + 1;
        } else {
          imageUrlList.push(u);
          firstFrameIdx = imageUrlList.length;
        }
      }
    }
    
    if (lastFrame) {
      const u = formatUrl(typeof lastFrame === 'string' ? lastFrame : (lastFrame.data || lastFrame.url));
      if (u) {
        const existingIdx = imageUrlList.indexOf(u);
        if (existingIdx !== -1) {
          lastFrameIdx = existingIdx + 1;
        } else {
          imageUrlList.push(u);
          lastFrameIdx = imageUrlList.length;
        }
      }
    }
    ...
    if (finalPrompt) {
      // 1. Replace explicit @图N labels
      finalPrompt = finalPrompt
        .replace(/@(?:图|Image|image)\s*(\d+)/g, '@Image $1')
        .replace(/@(?:视频|Video|video)\s*(\d+)/g, '@Video $1')
        .replace(/@(?:音频|Audio|audio)\s*(\d+)/g, '@Audio $1');

      // 2. Map history mentions if they were sent as referenceAssets
      // The frontend should have included these in referenceAssets, so mereka are already in the lists.
      ...
      // 3. Replace @首帧 / @尾帧 with actual indices
      if (firstFrameIdx !== -1) {
    ...
    if (workflowId && /^\d+$/.test(String(workflowId))) {
      ...
      body = {
        // Normalize prompt labels by removing spaces between @Type and Number (e.g. @Image 1 -> @Image1)
        prompt: finalPrompt.replace(/@(Image|Video|Audio)\s+(\d+)/gi, '@$1$2'),
        resolution: req.body.resolution || '720p',
        duration: String(duration || '5'),
        imageUrls: imageUrlList.slice(0, 9),
        videoUrls: videoUrlList.slice(0, 3), 
        audioUrls: audioUrlList.slice(0, 3), 
        generateAudio: true,
        ratio: ratio || 'adaptive',
        // realPersonMode: true + explicitly listing slots triggers the 0.4/s reference-based pricing
        realPersonMode: true, 
        conversionSlots: [
          'all',
          ...imageUrlList.slice(0, 9).map((_, i) => `image${i + 1}`),
          ...videoUrlList.slice(0, 3).map((_, i) => `video${i + 1}`),
          ...audioUrlList.slice(0, 3).map((_, i) => `audio${i + 1}`)
        ],
    ...
    try {
      ...
      if (!rhResponse.ok || (data && data.errorCode && data.errorCode !== '0' && data.errorCode !== '')) {
        let errorMsg = data.errorMessage || data.message || text;
        if (data.errorCode === '1014' || (typeof errorMsg === 'string' && errorMsg.includes('Enterprise-Shared'))) {
  ...
  app.listen(PORT, "0.0.0.0", () => {
    console.log(`>>> [SERVER] Web service listening on http://0.0.0.0:${PORT}`);
    fs.appendFileSync(path.join(process.cwd(), 'startup_debug.log'), `[SERVER] Listening on port ${PORT} at ${new Date().toISOString()}\n`);
  });
