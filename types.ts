...
export type ApiConfigKey = 'script' | 'image' | 'video' | 'videoVeoFast' | 'videoSeedance' | 'videoOmni' | 'gptImage';
...
export interface SmartImageConfig {
  ...
  gptQuality?: string;
  gptFormat?: string;
  referenceImages?: { id?: string; data: string; mimeType: string; type: 'style' | 'character' | 'environment' | 'general' | 'prop' }[];
  searchQuery?: string;
...
export interface SmartVideoConfig {
  ...
  referenceAssets?: { id?: string; data: string; thumbnailUrl?: string; mimeType: string; type: 'image' | 'video' | 'audio'; startTime?: number; duration?: number }[];
  customDescription?: string;
}
